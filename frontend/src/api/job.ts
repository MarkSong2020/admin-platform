...
export async function listJobs(params: JobListParams = {}): Promise<ScheduledTaskPage> {
  return unwrap(
    await apiClient.GET('/api/v1/monitor/jobs', {
      params: {
        query: {
          page: params.page,
          size: params.size,
...
export async function getJob(taskId: number): Promise<ScheduledTaskRead> {
  return unwrap(
    await apiClient.GET('/api/v1/monitor/jobs/{task_id}', {
...
export async function createJob(payload: ScheduledTaskCreate): Promise<ScheduledTaskRead> {
  return unwrap(await apiClient.POST('/api/v1/monitor/jobs', { body: payload }))
}
...
export async function updateJob(
  ...
  return unwrap(
    await apiClient.PATCH('/api/v1/monitor/jobs/{task_id}', {
      params: { path: { task_id: taskId } },
      body: payload,
    }),
  )
...
export async function deleteJob(taskId: number): Promise<void> {
  unwrap(
    await apiClient.DELETE('/api/v1/monitor/jobs/{task_id}', {
      params: { path: { task_id: taskId } },
    }),
  )
}
...
export async function listHandlers(): Promise<HandlerInfo[]> {
  return unwrap(await apiClient.GET('/api/v1/monitor/jobs/handlers'))
}

/** 手动触发执行（无 body）。返回本次执行日志；409 并发冲突。 */
export async function runJob(taskId: number): Promise<ScheduledTaskLogRead> {
  return unwrap(
    await apiClient.POST('/api/v1/monitor/jobs/{task_id}/run', {
      params: { path: { task_id: taskId } },
    }),
  )
}
...
export async function listJobLogs(params: JobLogListParams = {}): Promise<ScheduledTaskLogPage> {
  return unwrap(
    await apiClient.GET('/api/v1/monitor/jobs/logs', {
      params: {
        query: {
          page: params.page,
          size: params.size,
