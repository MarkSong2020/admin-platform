...
export async function listDepts(params: DeptListParams = {}): Promise<DeptPage> {
  ...
  const { data, error, response } = await apiClient.GET('/api/v1/depts', {
    params: { query },
  })
...
export async function getDept(deptId: number): Promise<DeptRead> {
  const { data, error, response } = await apiClient.GET('/api/v1/depts/{item_id}', {
...
export async function createDept(payload: DeptCreate): Promise<DeptRead> {
  const { data, error, response } = await apiClient.POST('/api/v1/depts', { body: payload })
  if (error !== undefined) throw toApiError(error, response)
  return data
}
...
export async function updateDept(deptId: number, payload: DeptUpdate): Promise<DeptRead> {
  const { data, error, response } = await apiClient.PATCH('/api/v1/depts/{item_id}', {
    params: { path: { item_id: deptId } },
    body: payload,
  })
  if (error !== undefined) throw toApiError(error, response)
...
export async function deleteDept(deptId: number): Promise<void> {
  const { error, response } = await apiClient.DELETE('/api/v1/depts/{item_id}', {
    params: { path: { item_id: deptId } },
  })
  if (error !== undefined) throw toApiError(error, response)
}
