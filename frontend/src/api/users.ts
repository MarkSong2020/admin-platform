...
/**
 * openapi-fetch 错误侧归一（同 auth.ts toApiError）：
...
export async function getUser(userId: number): Promise<UserRead> {
  const { data, error, response } = await apiClient.GET('/api/v1/users/{user_id}', {
...
export async function createUser(payload: UserCreate): Promise<UserRead> {
  const { data, error, response } = await apiClient.POST('/api/v1/users', { body: payload })
  if (error !== undefined) throw toApiError(error, response)
  return data
}
...
export async function updateUser(userId: number, payload: UserUpdate): Promise<UserRead> {
  const { data, error, response } = await apiClient.PATCH('/api/v1/users/{user_id}', {
    params: { path: { user_id: userId } },
    body: payload,
  })
  if (error !== undefined) throw toApiError(error, response)
...
export async function deleteUser(userId: number): Promise<void> {
  const { error, response } = await apiClient.DELETE('/api/v1/users/{user_id}', {
    params: { path: { user_id: userId } },
  })
  if (error !== undefined) throw toApiError(error, response)
}
...
export async function getUserRoles(userId: number): Promise<BindingRead> {
  const { data, error, response } = await apiClient.GET('/api/v1/users/{user_id}/roles', {
    params: { path: { user_id: userId } },
  })
  if (error !== undefined) throw toApiError(error, response)
  return data
...
export async function setUserRoles(userId: number, roleIds: number[]): Promise<void> {
  const { error, response } = await apiClient.PUT('/api/v1/users/{user_id}/roles', {
    params: { path: { user_id: userId } },
...
export async function getUserPosts(userId: number): Promise<BindingRead> {
  const { data, error, response } = await apiClient.GET('/api/v1/users/{user_id}/posts', {
    params: { path: { user_id: userId } },
  })
  if (error !== undefined) throw toApiError(error, response)
  return data
...
export async function setUserPosts(userId: number, postIds: number[]): Promise<void> {
  const { error, response } = await apiClient.PUT('/api/v1/users/{user_id}/posts', {
    params: { path: { user_id: userId } },
