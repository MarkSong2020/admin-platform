...
>
> **驱动更正(2026-06-24)**:本报告评估期以 `asyncmy` 为候选异步驱动,故下文实测多处标 `asyncmy`。
> 落地实现已改用 **`aiomysql`**(纯 PyMySQL,`pymysql>=1.1,<1.2`)——`asyncmy 0.2.11` 有 CVSS 9.3 SQL
> 注入 `CVE-2025-65896` 且 PyPI 无修复版、项目不活跃。实测结论(行锁去重、`DATETIME` naive 读回等)在
> SQLAlchemy / MySQL 层与具体驱动无关,仍成立;最终回归基线为 `mysql:8.0` + `aiomysql`。
