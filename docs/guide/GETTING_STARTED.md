...
> **数据库连接**：默认连 `mysql+asyncmy://app:app@localhost:3306/app`（与 `make compose-up` 起的容器一致）。要连别的库或开启鉴权，在仓库根建 `.env`（参照 `.env.example`），配置项统一带 `APP_` 前缀（如 `APP_DATABASE_URL`、`APP_AUTH_ENABLED`、`APP_AUTH_JWT_SECRET`）。
>
