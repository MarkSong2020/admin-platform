...
- **测试**：`make check` 761 ✓（fast lane：unit + api，ruff + pyright + pytest，不含 integration）/ `make test-integration` 208 ✓（需本地 DB + Redis）/ `make coverage` 门槛 85%（v0.0.1 起已接入 CI fast lane 强制）
...
make compose-up && make migrate    # 起 PostgreSQL + 应用 Alembic 迁移（当前 head 0020）
