...
> **这是什么**：单租户后台管理脚手架应用（`v0.0.1`，对标 RuoYi / 若依）。派生自脚手架模板 `python-web-service-template`（lineage v0.5.3）。
>
...
## 现状（v0.0.1 — P0–P6 全落地：后端 + Vue 前端，对标 RuoYi）

- **应用版本**：`v0.0.1`（`pyproject.toml [project].version`）。进度：JWT 认证 + Argon2 + user 五层 CRUD + CLI 建超管 ✓；P0.9 单租户回归 ✓；P1 RBAC（部门/角色/菜单/岗位 + RuoYi 数据权限 + getInfo/getRouters + audit_event.v1）✓；P1.4 登录增强（refresh 轮换/验证码/限流）✓；P1.5 安全加固 ✓；P2 审计持久化（audit_events + login_logs + IP/UA + operlog/logininfor 查询）✓；P3 字典·参数·通知 ✓；P4a–P4c 服务/缓存监控 + 在线用户 + 定时任务（APScheduler + DB leader election + DB claim + handler registry 白名单；MySQL GET_LOCK 迁移在阶段 3 落地）✓；P5 文件管理（对标 RuoYi sys_oss：`domains/file/` 五层 + `storage.py` StorageBackend 抽象/LocalFileStorage，零新依赖；扩展名白名单 + 魔数校验 + 流式上传下载 + 软删 + commit 后物理删；6 权限点 `system:file:*`）✓；P5 Excel 导入导出（对标 RuoYi 导入导出：通用 `admin_platform/excel/` reader/writer/schemas 零 domain 知识 + import-linter C10 叶子契约；post 绑定 import/export 2 端点 + formula injection 防御 + 一步全有全无 + 全量错误 + 2 权限点 `system:post:{import,export}`；openpyxl 3.1.5，无新迁移）✓。**P5 范围重新圈定（2026-06-11 拍板 + Codex PK）**：砍 RuoYi 在线 codegen + introspection 逆向（AI 时代过时），保留 `make new-module` CLI（agent 生成的「确定性护栏」，非 codegen）。P6 Vue 前端已落地（openapi-fetch 类型安全 + session 鉴权 + RBAC 动态路由）。对标路线图 → [`specs/2026-06-04-ruoyi-parity-roadmap.md`](archive/specs/2026-06-04-ruoyi-parity-roadmap.md)
...
make test-integration              # MySQL 集成测试
```
...
- SDK 自动生成 / Sentry / OTel Collector / K8s manifests / Helm chart
- 内部 RPC（gRPC / Thrift）—— 团队不用
- 业务专属 channels / 风控 / 合规等垂直域代码

## 维护者

- 作者 / 维护：Mark Song（[@MarkSong2020](https://github.com/MarkSong2020)）
- 许可：[MIT](../LICENSE)
...
→ [CHANGELOG](../CHANGELOG.md) 脚手架 lineage 与各阶段演进史
