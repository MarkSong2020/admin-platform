...
> 决策与拆除背景见 [`../specs/2026-06-04-ruoyi-parity-roadmap.md`](../archive/specs/2026-06-04-ruoyi-parity-roadmap.md) §3「单租户回归重构」。
>
> ---
...
机制在 `db/tenant_filter.py`（已随多租户拆除删除），注册到
...
- `tenants` 表（原 `domains/tenant/models.py`，已删）是**平台级表**（**不**带 `TenantMixin`，
...
见 `tests/integration/test_tenant_isolation.py`（已删）
+ `test_user_crud.py`（含跨租户删→404）；fail-closed 单测见
`tests/unit/test_tenant_filter.py`（已删）。
