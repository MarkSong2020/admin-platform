...
| Q11 | 初始菜单/权限/角色/超管 seed 从哪来：migration seed / CLI seed / fixture seed / 独立 bootstrap 命令？| **P1.0/P1.1** |
| Q12 | 权限命名规范 + resource/permission registry：`system:user:list` 谁维护、如何防重复/漂移 | **P1.0** |
...
| Q16 | 审计事件 envelope：P1 就冻结，至少覆盖权限拒绝/登录失败/RBAC 写操作 | **P1.0** |
| Q17 | P6 前端契约产物：OpenAPI 子集 / 示例 JSON / contract test / mock server | **P1 冻结形态** |
