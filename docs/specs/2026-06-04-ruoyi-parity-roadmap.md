...
| RuoYi 模块 | admin 现状 | gap | 阶段 |
|---|---|---|---|
| 代码生成 | ✅ **codegen 砍除（2026-06-11 拍板）**；保留 `make new-module`（CLI 生成五层 domain） | RuoYi 在线 codegen（读库表 → 生成 CRUD）+ introspection 逆向**已决定不对标**——AI 时代过时（coding agent 直接读表生成五层 CRUD + 测试 + 迁移 + doc，比 velocity 模板灵活；绿地项目无遗留表逆向场景）。`make new-module` 保留作 agent 生成的「确定性护栏」（结构/import-linter/schema-doc 自动注册），非 codegen。详见 §7 Q2 | ✅ |
...
| 多租户文档 | `docs/architecture/MULTI_TENANCY.md` + ADR-A/B/E | 归档为"已废弃方向"，记决策原因 |
...
P5   工具                    codegen 砍除（AI 时代，§7 Q2）+ 文件管理 ✅（对标 RuoYi sys_oss）+ Excel 导入导出 ✅（往返一致，对标 RuoYi 导入导出）
...
| P5 | ~~给一张表生成可用 CRUD 代码（对标 RuoYi gen）~~ **codegen 砍除（AI 时代，§7 Q2）**；文件管理上传/下载/软删往返一致（对标 RuoYi sys_oss）✅；Excel 导入导出往返一致（对标 RuoYi 导入导出，post 绑定第一版）✅ → [spec](./2026-06-11-p5-excel-import-export.md)|
...
- **P5 红线**（2026-06-11 修订）：~~codegen 输出自带 tests / migration / doc / schema-doc 挂钩~~ **在线 codegen 砍除（§7 Q2）**——`make new-module` 保留作 agent 生成护栏（仍守 import-linter / schema-doc / column-comment 自动注册）；P5 实际交付 = **文件管理**（对标 RuoYi sys_oss，已落地）+ **Excel 导入导出**（对标 RuoYi 导入导出，已落地，[spec](./2026-06-11-p5-excel-import-export.md)）。文件管理红线：defense-in-depth 安全模型（扩展名白名单 + 魔数校验 + 不信 Content-Length 边写边累计 size/sha256 + 路径穿越守卫 + Content-Disposition 注入防御 + nosniff）+ 软删元数据 + commit 后物理删（防 commit-after-unlink 数据丢失）。Excel 红线：通用 `admin_platform/excel/` 顶层叶子（import-linter C10 禁 fastapi/sqlalchemy/domains/core）+ 导出 formula injection 防御（=/+/-/@ 前缀文本化）+ import 流式 size 上限（不 OOM）+ 一步全有全无 + 行级全量错误（200+summary.errors，非 422 因 ProblemDetail.errors 受 debug 脱敏）。
...
| Q2 | 代码生成器对标：admin 的 `make new-module`（模板驱动 CLI）vs RuoYi 的"读库表在线生成"——是改造 generator 支持读表，还是保持 CLI 不强对标？ | ✅ **已定（2026-06-11 拍板 + Codex PK）**：**砍** RuoYi 在线 codegen + introspection 逆向（AI 时代过时——coding agent 直接读表生成五层 CRUD + 测试 + 迁移 + doc 比 velocity 模板灵活，绿地项目无遗留表逆向场景）；**保留** `make new-module` CLI（它不是 codegen，是 agent 生成时的「确定性护栏」，AI 时代防结构漂移反而更有用）。「新模块必走 make new-module」规则不变 |
...
- **codegen 在 AI 时代降权为 CRUD 工具、升权为工程纪律守门**（强制五层 + 迁移/测试/docs/openapi + 自动纳入 import-linter + 输出权限矩阵测试骨架）——"工程纪律放大器，不是 boilerplate 打字机"。
