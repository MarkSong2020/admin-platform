...
P5   工具                    codegen 砍除（AI 时代，§7 Q2）+ 文件管理 ✅（对标 RuoYi sys_oss）+ Excel 导入导出（待依赖授权）
...
| P5 | ~~给一张表生成可用 CRUD 代码（对标 RuoYi gen）~~ **codegen 砍除（AI 时代，§7 Q2）**；文件管理上传/下载/软删往返一致（对标 RuoYi sys_oss）✅；Excel 导入导出往返一致（待依赖授权）|
...
- **P5 红线**（2026-06-11 修订）：~~codegen 输出自带 tests / migration / doc / schema-doc 挂钩~~ **在线 codegen 砍除（§7 Q2）**——`make new-module` 保留作 agent 生成护栏（仍守 import-linter / schema-doc / column-comment 自动注册）；P5 实际交付 = **文件管理**（对标 RuoYi sys_oss，已落地）+ **Excel 导入导出**（待依赖授权）。文件管理红线：defense-in-depth 安全模型（扩展名白名单 + 魔数校验 + 不信 Content-Length 边写边累计 size/sha256 + 路径穿越守卫 + Content-Disposition 注入防御 + nosniff）+ 软删元数据 + commit 后物理删（防 commit-after-unlink 数据丢失）。
