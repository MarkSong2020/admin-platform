...
> admin-platform（单租户后台管理脚手架，对标 RuoYi）的全部文档入口。**按角色找最快路径**——先看你是谁，再点链接。
...
make migrate       # 应用 Alembic 迁移（当前 head 0019；0013–0019 仅本地 dev + CI 跑过，生产迁移 gated）
...
> 当前 P0–P4 全落地（auth + RBAC + 审计持久化 + 字典·参数·通知 + 监控·在线用户·定时任务，单租户）+ P5 文件管理已落地（对标 RuoYi sys_oss，spec [`specs/2026-06-11-p5-file-management.md`](specs/2026-06-11-p5-file-management.md)）+ P5 Excel 导入导出已落地（对标 RuoYi 导入导出，通用 `admin_platform/excel/` + post 绑定，spec [`specs/2026-06-11-p5-excel-import-export.md`](specs/2026-06-11-p5-excel-import-export.md)）；P5 范围 codegen 砍除（AI 时代），下一步 P6 Vue 前端（技术栈选型与架构设计稿 v2 已出，经 Codex high 对抗 review，待落地，spec [`specs/2026-06-11-p6-frontend-design.md`](specs/2026-06-11-p6-frontend-design.md)）。对标路线图见 [`specs/2026-06-04-ruoyi-parity-roadmap.md`](specs/2026-06-04-ruoyi-parity-roadmap.md)。
