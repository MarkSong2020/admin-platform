...
> **结构边界**（import 方向、api↛repository、schemas↛models、service/repository↛fastapi、models↛pydantic）由 `make check` 的 **import-linter 机检**（契约 C1–C8，见 `.importlinter`）——直接跨层 import 会让 CI 红。
...
> **C1–C8 全部生效**（`make check` 8 contracts kept）：`user` 域已完整五层、纳入 C1 `containers`；`auth` 仅 service/schemas（router 在 `api/v1/auth.py` 顶层）暂不纳入 C1，待长出三层再加；`tenant` 域已随单租户回归（2026-06-05）删除。C2–C7 用 wildcard 覆盖全部域；C8 守 `authz` 纯基座（不 import domains / core，避免循环依赖）。
