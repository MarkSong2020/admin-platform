...
- **多租户隔离机制（写 repo / 查询前必读）** → [architecture/MULTI_TENANCY.md](./architecture/MULTI_TENANCY.md)（fail-closed 隔离 + system_session 用法 + repo 隔离正确性红线：select 非 session.get / UoW 非 bulk DML）
