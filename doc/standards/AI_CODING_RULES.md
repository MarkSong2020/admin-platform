...
> - 跨语言协同约定 → `team-engineering-adr/0001-cross-language-conventions.md`
...
| 改了什么代码 | 同步改 |
|---|---|
| `core/*.py` | `doc/architecture/<对应主题>.md` |
| `scripts/new_module.py` 模板 | `doc/standards/CODE_GENERATOR.md` |
| 加表 / 改列（`domains/*/models.py`） | `make schema-doc` 重生 `doc/architecture/DATA_MODEL.md`（生成物，勿手改） |
| 加新 Settings 字段 | `doc/architecture/REQUEST_LIFECYCLE.md` 配置表 |
| 新加 Make target | `doc/PROJECT_OVERVIEW.md` 快速命令段 |
| 解决某项 known deviation | `doc/tech-debt/KNOWN_DEVIATIONS.md` 划除该项 |
| ADR Open Q 决议 | `doc/tech-debt/OPEN_QUESTIONS.md` 划除该项 |
...
- Java/Python 跨语言协同（看 ADR `team-engineering-adr/0001-*.md`，本仓 stub 在 [../reference/CROSS_LANGUAGE_ADR.md](../reference/CROSS_LANGUAGE_ADR.md)）
