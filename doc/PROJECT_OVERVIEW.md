...
- **测试**：`make check` 621 ✓（fast lane：unit + api，ruff + pyright + pytest，不含 integration）/ `make test-integration` 207 ✓（需本地 DB + Redis）/ `make coverage` 门槛 85%（v0.0.1 起已接入 CI fast lane 强制）
