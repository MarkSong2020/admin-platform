"""/api/v1/roles 的 API 测试 —— 权限守卫接线 + 校验（DB-free）。
...
完整 CRUD happy / NOT_FOUND / 归一范围 路径放 ``tests/integration/test_role_crud.py``（真 DB）。
role router 挂进生产 ``create_app()`` 是人值守红线（``main.py``），落地后补一条「mounted」回归。
"""
...
def test_list_without_auth_returns_401() -> None:
    ...
    res = _client(current_user=None, provider=None).get("/api/v1/roles")
    assert res.status_code == 401


def test_list_without_permission_returns_403() -> None:
    res = _no_perm_client().get("/api/v1/roles")
    assert res.status_code == 403
    assert res.json()["type"] == "auth.FORBIDDEN_BY_ROLE"


def test_query_without_permission_returns_403() -> None:
    assert _no_perm_client().get("/api/v1/roles/1").status_code == 403


def test_add_without_permission_returns_403() -> None:
    res = _no_perm_client().post("/api/v1/roles", json={"name": "x", "code": "X"})
    assert res.status_code == 403


def test_edit_without_permission_returns_403() -> None:
    assert _no_perm_client().patch("/api/v1/roles/1", json={"name": "x"}).status_code == 403
...
def test_remove_without_permission_returns_403() -> None:
    ...
    client = _client(
        current_user=CurrentUser(user_id="2", sub="2"),
        provider=_StubProvider(is_super=False, perms=frozenset({"system:role:list"})),
    )
    assert client.delete("/api/v1/roles/1").status_code == 403
...
def test_create_returns_422_on_missing_field() -> None:
    # 缺 name / code 必填字段。
    res = _superadmin_client().post("/api/v1/roles", json={})
    assert res.status_code == 422


def test_create_invalid_data_scope_rejected_422() -> None:
    # data_scope 非 5 范围之一 → Pydantic Literal 拒绝。
    res = _superadmin_client().post(
        "/api/v1/roles", json={"name": "x", "code": "X", "data_scope": "bogus"}
    )
    assert res.status_code == 422


def test_update_invalid_status_rejected_422() -> None:
    res = _superadmin_client().patch("/api/v1/roles/1", json={"status": "bogus"})
    assert res.status_code == 422
