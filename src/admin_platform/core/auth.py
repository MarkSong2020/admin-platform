...
def is_public_path(path: str, public_paths: Iterable[str]) -> bool:
    """路径是否命中公开前缀。鉴权中间件与 OpenAPI security 标注共用同一判定，避免「精确匹配
    vs 前缀匹配」两套口径漂移（Workflow 深审）：若将来 ``auth_public_paths`` 加入目录式前缀
    （如 ``/api/v1/public``），其子路径在中间件免鉴权、OpenAPI 却标 bearerAuth 的反向分叉。
    命中口径：精确相等 / 子路径（``p/``）/ 带 query（``p?``）。
    """
    return any(
        path == p or path.startswith(p + "/") or path.startswith(p + "?") for p in public_paths
    )
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def _is_public_path(self, path: str) -> bool:
        return is_public_path(path, self._public_prefixes)
