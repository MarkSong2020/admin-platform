...
- **单 worker per pod**：扩容靠 pod 数 + HPA，不靠 `uvicorn --workers N`（见 [`../../doc/operations/DEPLOYMENT.md`](../../doc/operations/DEPLOYMENT.md)）
- **`APP_STARTUP_EAGER_CONNECT=true`**：生产强制 eager-probe DB+Redis，连不上 pod 不 ready（见 [`../../doc/architecture/REQUEST_LIFECYCLE.md`](../../doc/architecture/REQUEST_LIFECYCLE.md)）
